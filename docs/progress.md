...
| T5 | 冷却状态持久化 | 现在冷却只在进程内，重启会重置。真被封号时重启一次就又去捅了 |
| T6 | 并发抓取 | 22 源串行，冷启动约 40s。源越多越难看，且调度器一轮要跑很久 |
| T7 | 数据清理策略 | `items` 表只增不删，跑久了会一直长 |
