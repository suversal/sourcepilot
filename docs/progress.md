...
| T5 | 声明式引擎要能识别「业务错误码」 | 很多站点用 HTTP 200 + 响应体里的错误码表示拒绝（B站 `code`、公众平台 `base_resp.ret`）。现在引擎只看 HTTP 状态，遇到这种一律报「多半是对方改版了」——实测 B站就误报过一次，其实只是瞬时被拒（复测 5/5 正常）。误导排查方向，也让 Canary 无法区分「结构变了」和「临时挡了一下」 |
| T6 | 冷却状态持久化 | 现在冷却只在进程内，重启会重置。真被封号时重启一次就又去捅了 |
| T7 | 并发抓取 | 22 源串行，冷启动约 40s。源越多越难看，且调度器一轮要跑很久 |
| T8 | 数据清理策略 | `items` 表只增不删，跑久了会一直长 |
