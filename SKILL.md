...
默认地址 `http://127.0.0.1:8420`（远程走 Tailscale 时换成对应主机名）。
下文的 `BASE` 指这个地址。

**第一次调用前先确认连对了服务**：`GET BASE/` 应该返回
`{"name": "SourcePilot", "contract_version": ...}`。返回别的东西说明端口上
跑的是另一个服务——**不要继续请求**，告诉用户地址不对。这不是多余的谨慎：
开发机上 8000 之类的常用端口经常被别的程序占着，打错了会拿到不相干的数据。
...
| 「量子位/机器之心 最近发了什么」（公众号） | `GET BASE/api/v1/wechat/feed?window=7d` |
...
**不要猜端点。** 上表就是全部能力。用户要的东西不在表里（比如小红书、微博、
知乎），直接说「这个信源还没接」，**不要编一个 URL 去试，也不要改用别的来源冒充**
——拿 B站热榜回答「微博上在说什么」比说「没接」更糟。

## 摘要不全时怎么办

约 60% 的条目有摘要。**今日头条、Hacker News、LINUX DO、掘金这四个源几乎全是
只有标题**（它们的列表页就不提供描述），加上 B站与 DeepSeek、字节 Seed 的一部分。
（比例会随各源产量变化，以实际响应为准，别把这个数字当承诺。）
...
`get_wechat_feed` 只覆盖**已订阅的号**（当前：量子位、机器之心），不是全网搜索。
用户问一个没订阅的号，如实说「没订阅这个号」，别拿别家的文章顶替。
它的 `score` 恒为 0——订阅流不是排行榜，没有热度可言。
